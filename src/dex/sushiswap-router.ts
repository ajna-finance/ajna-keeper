...
export async function swapWithSushiswapRouter(
  ...
  if (!swapRouterAddress) {
    throw new Error(
      'SushiSwap Router address must be provided via configuration'
    );
  }
  if (!feeTier) {
  ...
  if (!factoryAddress) {
    throw new Error(
      'SushiSwap factory address must be provided via configuration'
    );
  }
  ...
  try {
    ...
    if (poolAddress === '0x0000000000000000000000000000000000000000') {
      throw new Error(
        `No SushiSwap pool exists for ${tokenToSwap.symbol}/${targetToken.symbol} with fee ${feeTier}`
      );
    }
    logger.info(
      `Found SushiSwap pool at ${poolAddress} for ${tokenToSwap.symbol}/${targetToken.symbol}`
    );
    ...
    logger.info(
      `Input amount: ${weiToDecimaled(amount, tokenToSwap.decimals)} ${tokenToSwap.symbol}`
    );
    logger.info(
      `Minimum output with ${slippagePercentage}% slippage: ${weiToDecimaled(minAmountOut, targetToken.decimals)} ${targetToken.symbol} (conservative estimate)`
    );
    ...
    logger.info(
      `Current SushiSwap router allowance: ${weiToDecimaled(currentAllowance, tokenToSwap.decimals)} ${tokenToSwap.symbol}`
    );
    ...
    if (currentAllowance.lt(amount)) {
      ...
      logger.info(
        `SushiSwap router already has sufficient allowance for ${tokenToSwap.symbol}`
      );
    }
    ...
    logger.debug(
      `   amountIn: ${weiToDecimaled(swapParams.amountIn, tokenToSwap.decimals)}`
    );
    logger.debug(
      `   amountOutMinimum: ${weiToDecimaled(swapParams.amountOutMinimum, targetToken.decimals)}`
    );
    logger.debug(
