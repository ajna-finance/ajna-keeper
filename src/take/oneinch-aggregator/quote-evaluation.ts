...
export async function getOneInchAggregatorPathQuoteEvaluation(
  ...
  try {
    ...
    logger.info(
      `1inch aggregator take check for pool ${pool.name}: marketPrice=${economics.marketPrice.toFixed(6)}, takeablePrice=${economics.takeablePrice.toFixed(6)}, auctionPrice=${price.toFixed(6)}, collateral=${economics.collateralAmount}, factor=${poolConfig.take.marketPriceFactor}, expectedOutputRaw=${approvedQuote.quoteAmountRaw.toString()}, routeMinOutRaw=${approvedQuote.routeMinOutRaw.toString()}, approvedMinOutRaw=${policy.approvedMinOutRaw.toString()}, target=${approvedQuote.transactionTarget}, approvalSpender=${approvedQuote.approvalSpender}, selector=${approvedQuote.selector}, rejectionReason=${policy.rejectionReason ?? 'none'} -> ${policy.isEconomicallyExecutable ? 'TAKEABLE' : 'skip'}`
    );
