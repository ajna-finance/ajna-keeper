...
| **Daemon lifecycle** | ✅ (lifecycle scenario) persistent process, `while(true)` loops, `delayBetweenRuns` cadence, multi-cycle, real-subgraph discovery, cross-cycle idempotency, SIGTERM graceful shutdown, gas-spike skip, read-RPC cooldown/recovery; degraded-mode continuity covered at the unit layer (`discovery-runtime.test.ts`) | ❌ still: fork-level crash-injection across all 5 loops, daemon-fork degraded-mode injection, nonce-consistency during the in-flight SIGTERM submit window (post-dispatch, pre-acceptance) |
