...
describe('Curve Quote Provider', () => {
  ...
  describe('Curve Configuration Variations', () => {
    ...
    it('should validate Base mainnet pool addresses are real', async () => {
      ...
      >;
      const poolAddresses = Object.values(poolConfigs).map(config => config.address);
      const tokenAddresses = Object.values(validConfig.tokenAddresses as Record<string, string>);
