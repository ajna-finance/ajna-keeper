// tests/integration/curve-quote-provider.test.ts
...
describe('Curve Quote Provider', () => {
  ...
  it('should create Curve quote provider with valid pool configurations', async () => {
    ...
    expect(configuredPools).to.include(
  ...
  it('should handle missing tokenAddresses configuration', async () => {
    ...
    quoteProvider = new CurveQuoteProvider(
      mockSigner,
      configWithoutTokenAddresses
    );
  ...
  describe('Curve Configuration Variations', () => {
    it('should handle different pool type configurations', async () => {
      ...
      const stableProvider = new CurveQuoteProvider(
        mockSigner,
        stableOnlyConfig
      );
      ...
      const cryptoProvider = new CurveQuoteProvider(
        mockSigner,
        cryptoOnlyConfig
      );
    ...
    it('should validate Base mainnet pool addresses are real', async () => {
      ...
      const poolConfigs = validConfig.poolConfigs as Record<
        string,
        { address: string; poolType: CurvePoolType }
      >;
      const poolAddresses = Object.values(poolConfigs).map(
        (config) => config.address
      );
      const tokenAddresses = Object.values(
        validConfig.tokenAddresses as Record<string, string>
      );

      [...poolAddresses, ...tokenAddresses, validConfig.wethAddress].forEach(
        (address) => {
          expect(
            ethers.utils.isAddress(address),
            `${address} should be valid address`
          ).to.be.true;
          expect(address, `${address} should not be zero address`).to.not.equal(
      ...
      Object.values(poolConfigs).forEach((config) => {
        expect([CurvePoolType.STABLE, CurvePoolType.CRYPTO]).to.include(
          config.poolType
        );
      });
    });
