...
14. Keep `maxConcurrentCandidateEvaluations` unset (default `1`) until you have measured provider latency. Values up to `4` evaluate a same-pool candidate window concurrently, but execution remains serial with fresh final revalidation. If you raise it above `1`, set `maxInFlightRouteProbes` to cap combined 1inch and factory quote/API/RPC pressure across the window; `maxInFlightRouteProbes` defaults to `3` when unset and is capped at `16`.
15. Keep `maxExecutionsPerPoolPerRun` unset (default `1`) for conservative operation. Set it up to `5` for defensive same-pool liquidation cascades; this keeps executions serialized and refreshes auction state, route approval, and gas policy before every transaction while reusing warm route/provider caches.
16. Keep the hot-auction cache enabled for fast take loops. Defaults are conservative; set `discovery.take.hotAuctionCandidateTtlMs` to shorten/extend the window and `maxHotAuctionCandidates` to bound memory. Set the TTL to `0` only if you intentionally want each take loop to depend solely on the latest subgraph response.
17. Use `discovery.take.externalTakeTransportPolicy: 'require_private_or_relay'` for live production discovered external takes only after `writes.take` is configured for `private_rpc` or `relay`. `prefer_private_or_relay` warns but still allows public fallback.
18. If `allowedExternalTakePaths` includes both `'oneinch'` and `'factory'`, set `defaultFactoryLiquiditySource` and `validateRouteDeployments: true` so the factory selector has a default source and startup verifies the taker path.
19. If you use Curve for discovered takes, include both `dex.curve.poolConfigs` and `network.tokenAddresses`, or config validation will reject startup.
20. Use `discovery.take.validateRouteDeployments: true` for production startup preflight. It is required for hybrid 1inch/factory routing and recommended for every live factory route.
...
| `discovery.take.maxExecutionsPerPoolPerRun`       | `1`                                               | Allows defensive same-pool cascades to execute more than one borrower in a single discovered take cycle. Values above 1 force same-pool candidate evaluation back to sequential mode so stale pre-execution status is not reused after a state-changing take.                                                                                                                                                                                  |
