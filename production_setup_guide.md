...
15. Keep the hot-auction cache enabled for fast take loops. Defaults are conservative; set `discovery.take.hotAuctionCandidateTtlMs` to shorten/extend the window and `maxHotAuctionCandidates` to bound memory. Set the TTL to `0` only if you intentionally want each take loop to depend solely on the latest subgraph response.
16. Use `discovery.take.externalTakeTransportPolicy: 'require_private_or_relay'` for live production discovered external takes only after `writes.take` is configured for `private_rpc` or `relay`. `prefer_private_or_relay` warns but still allows public fallback.
17. If `allowedExternalTakePaths` includes both `'oneinch'` and `'factory'`, set `defaultFactoryLiquiditySource` and `validateRouteDeployments: true` so the factory selector has a default source and startup verifies the taker path.
18. If you use Curve for discovered takes, include both `dex.curve.poolConfigs` and `network.tokenAddresses`, or config validation will reject startup.
19. Use `discovery.take.validateRouteDeployments: true` for production startup preflight. It is required for hybrid 1inch/factory routing and recommended for every live factory route.
