...
10. Leave `externalTakeRouteSelectionMode` unset for `maximize_profit`, which probes all enabled paths and ranks by expected net profit. Use `'cost_aware'` only when reducing 1inch API calls is worth potentially skipping a better 1inch route; it tries factory first and stops once factory is approved.
...
- Can reduce 1inch API usage with `externalTakeRouteSelectionMode: 'cost_aware'` by trying factory before 1inch
