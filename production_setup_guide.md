...
14. Keep `maxConcurrentCandidateEvaluations` unset or `1` until you have measured provider latency. Values up to `4` evaluate a same-pool candidate window concurrently, but execution remains serial with fresh final revalidation. If you raise it above `1`, use `maxInFlightRouteProbes` to cap combined 1inch and factory quote/API/RPC pressure across the window; unset defaults to `3`.
