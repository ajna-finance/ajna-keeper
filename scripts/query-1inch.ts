...
const PATH_TO_COMPILER_OUTPUT =
  'artifacts/contracts/AjnaKeeperTaker.sol/AjnaKeeperTaker.json';
...
async function main() {
  ...
  if (argv.action === 'deploy') {
    ...
    const keeperTakerFactory: ContractFactory = ContractFactory.fromSolidity(
      compilerOutput,
      signer
    );
  ...
  if (argv.action === 'approve' && pool && dexRouter) {
    ...
    if (currentAllowance.lt(amount)) {
      try {
        console.log(
          `Approving 1inch router ${oneInchRouter} for token: ${pool.collateralAddress}`
        );
        await approveErc20(
          signer,
          pool.collateralAddress,
        ...
        console.error(
          `Failed to approve token ${pool.collateralAddress} for 1inch: ${error}`
        );
      }
    }
    ...
    console.log(
      'Expected for USDC (6 decimals):',
      '10^12 =',
      Math.pow(10, 12).toString()
    );
    console.log('Pool quote scale:', quoteScale.toString());
    console.log(
      'Expected for savUSD (18 decimals):',
      '10^18 =',
    ...
    if (config.takers.oneInch) {
      ...
      const tx = await keeperTaker.testOneInchSwapBytes(
        ...
        encodeOneInchSwapDetailsBytes(
          convertSwapApiResponseToDetails(swapData.data)
        ),
